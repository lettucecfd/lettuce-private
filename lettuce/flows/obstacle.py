...
class Obstacle:
    ...
    Attributes
    ----------
    mask : torch.Tensor with dtype = bool
        Boolean mask to define the obstacle. The shape of this object is the
        shape of the grid. Initially set to zero (no obstacle).
   ...
   """
    ...
    def initial_solution(self, x):
        p = torch.zeros_like(x[0], device=self.units.lattice.device)[None, ...]
    ...
    def _unit_vector(self, i=0):
        return torch.eye(self.units.lattice.D,
                         device=self.units.lattice.device)[i]


def Obstacle2D(resolution_x, resolution_y, reynolds_number, mach_number,
               lattice, char_length_lu):
    ...
    return Obstacle(shape, reynolds_number, mach_number, lattice,
                    domain_length_x=domain_length_x)


def Obstacle3D(resolution_x, resolution_y, resolution_z, reynolds_number,
